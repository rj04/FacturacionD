<?php
// c:\UniServerZ\www\inventario-test\registrarEstados_dt.php
...
$orderColumnIndex = isset($_GET['order'][0]['column']) ? intval($_GET['order'][0]['column']) : 1; // Ordenar por estado por defecto
...
if (!empty($searchValue)) {
    $whereSql = " WHERE LOWER(estado) LIKE LOWER(?) ";
    $params[] = "%" . $searchValue . "%";
    $types .= "s";
}

// --- Contar Total de Registros (Sin Filtros) ---
$totalRecordsSql = "SELECT COUNT(idEstado) as total " . $baseSql;
...
$filteredRecordsSql = "SELECT COUNT(idEstado) as total " . $baseSql . $whereSql;
...
if ($stmtFiltered) {
    ...
    } else { error_log("Error ejecutando conteo filtrado en registrarEstados_dt.php: " . $stmtFiltered->error); }
    $stmtFiltered->close();
} else { error_log("Error preparando conteo filtrado en registrarEstados_dt.php: " . $mysqli->error); }

// --- Construir Consulta Principal para Obtener Datos ---
$dataSql = "SELECT idEstado, estado " . $baseSql . $whereSql
...
if ($stmtData) {
    ...
    if ($stmtData->execute()) {
        ...
        else { error_log("Error obteniendo resultados de datos en registrarEstados_dt.php: " . $mysqli->error); }
    } else { error_log("Error ejecutando consulta de datos en registrarEstados_dt.php: " . $stmtData->error); }
    $stmtData->close();
} else { error_log("Error preparando consulta de datos en registrarEstados_dt.php: " . $mysqli->error); }
