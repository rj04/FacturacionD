...
<div class="content-wrapper"> 
    <div class="container">
        <div class="content-header">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0" style="font-size:20px">Lista de Residentes</h1>
                </div>
                <div class="col-sm-6 text-sm-right mt-2 mt-sm-0">
                    <a href="agregarResidente.php" class="btn btn-primary">Agregar Nuevo Residente</a>
                </div>
            </div>
        </div>
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-body">
                            ...
                            <div class="row mb-3">
                                <div class="col-md-6">
                                    <label for="customSearchInput">Buscar Residente:</label>
                            ...
                            <div class="table-responsive">
                                <table id="residentesTable" class="table table-bordered table-striped" width="100%">
                                    <thead>
                                        <tr>
                                            <th>Residente</th>
                                            <th>Teléfono</th>
...
$(document).ready(function() {
    ...
    var table = $('#residentesTable').DataTable({
        "processing": true,
        "serverSide": true,
        "ajax": {
            "url": "listarResidentes_dt.php", // Nuevo script backend
            "type": "GET",
        ...
        // *** Modificar DOM para quitar la 'f' (filtro/search) por defecto y otros elementos si es necesario ***
        ...
        "columns": [
            ...
            {
                "data": "idResidente", // ID para acciones
                "orderable": false,
                "searchable": false,
                "width": "15%",
                "render": function(data, type, row) {
                    let editUrl = `editarResidente.php?idResidente=${data}`;
        ...
        "language": {
            ...
            "emptyTable": "No hay residentes registrados",
            "zeroRecords": "No se encontraron residentes"
        },
        "responsive": true,
        "autoWidth": false,
        "pageLength": 10,
        "lengthMenu": [[10, 25, 50, -1], [10, 25, 50, "Todos"]],
        "order": [[ 1, "asc" ]] // Ordenar por nombre de residente por defecto
    });

    // *** INICIO: Eventos para búsqueda personalizada ***
    let debounceTimer; // Variable para el temporizador del debounce

    // Evento click en el botón de búsqueda (opcional, pero útil)
...
function confirmarEliminacion(idResidente) {
    Swal.fire({
        title: '¿Estás seguro?',
        text: "¡No podrás revertir la eliminación de este residente!",
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            fetch('eliminarResidente.php', { // Nuevo script para eliminar
                method: 'POST',
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                },
                body: `idResidente=${idResidente}` // Enviar ID
            })
            .then(response => response.json())
            .then(data => {
                if (data.success) {
                    Swal.fire('¡Eliminado!', 'El residente ha sido eliminado.', 'success');
                    $('#residentesTable').DataTable().ajax.reload(null, false); // Recargar sin resetear paginación
                } else {
                    Swal.fire('Error', data.message || 'No se pudo eliminar el residente.', 'error');
                }
            })
            .catch(error => {
