...
// Función para extraer y validar el código del proyecto
function extraerCodigoProyecto($nombreArchivo) {
    logMessage("Intentando extraer código de proyecto de: " . $nombreArchivo);
    ...
    $patrones = [
        '/^(A\d{7})\s*-/',        // Formato: A2022001 -
        '/^(S\d{4}-\d{4})\s*-/',  // Formato: S0176-2024 -
        '/^(S\d{4}-\d{4})\s/',    // Formato: S0176-2024 
        '/^(S\d{4})-(\d{4})\s*/', // Formato alternativo con guión
        '/^(S\d{4})(\d{4})\s*/',  // Formato sin guión
        '/^([AS]\d{4,})-?(\d{4})\s*/', // Formato más flexible
        '/^(A\d{7})\s/',          // Formato: A2022001 sin guión
        '/^([AS]\d{7})\s*/'       // Formato más flexible para códigos de 7 dígitos
    ];
    
    foreach ($patrones as $patron) {
        if (preg_match($patron, $nombreSinExtension, $matches)) {
            // Si encontramos dos grupos, asegurarnos de que estén correctamente formateados
            if (isset($matches[2])) {
            ...
            logMessage("Código de proyecto extraído: " . $codigo);
            return $codigo;
        }
    }
    
    // Si no se encontró ningún patrón válido
    $mensaje = "No se pudo extraer un código de proyecto válido del archivo: $nombreArchivo";
    logMessage($mensaje);
...
function validarProyecto($codigo) {
    ...
    $query = "SELECT idProyecto FROM proyectos WHERE codigoProyecto = ?";
...
function extraerComentarios($archivo) {
    $comentarios = [];
