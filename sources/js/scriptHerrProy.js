document.getElementById('searchHe').addEventListener('input', function() {
    let query = this.value.trim();
    if (query.length >= 2) {
        fetch(`buscarHP.php?q=${query}`)
            .then(response => {
                if (!response.ok) {
            ...
            .then(data => {
                let resultsTbody = document.getElementById('resultHe'); 
                ...
                data.herramientas.forEach(item => {
                    ...
                    row.innerHTML = `
                        ...
                        <td>${item.oca}</td>
                        <td> 
                            <a href="editarHerramientas.php?idHerramienta=${item.idHerramienta}"><i class="bi bi-pencil"></i> Editar</a>
                        </td>
                        <td>
                            <a href="#" onclick="eliminarHerramienta(${item.idHerramienta})"><i class="bi bi-trash"></i> Eliminar</a>
                        </td>
                    `;
                ...
                paginationUl.querySelectorAll('a.page-link').forEach(link => {
                    link.addEventListener('click', function(e) {
                        ...
                        fetch(`buscarHerramientas.php?q=${query}&page=${page}`)
        ...
        document.getElementById('resultHe').innerHTML = '';
    }
});
...
function eliminarHerramienta(id) {
    if (confirm('¿Está seguro de que desea eliminar esta herramienta?')) {
        fetch('eliminarHerramientas.php', {
            ...
            body: JSON.stringify({ idHerramienta: id })
        })
        .then(response => response.json())
        .then(data => {
            if (data.success) {
