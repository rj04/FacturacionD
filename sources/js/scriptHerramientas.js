document.addEventListener("DOMContentLoaded", function() {
    ...
    const table = $('#herramientasTable').DataTable({
        processing: true,
        serverSide: true,
        ajax: {
        ...
        columns: [
             ...
             // Nueva columna para ver las transferencias
            {
                data: "idHerramienta",
                render: function(data) {
                    return `<a href="detalleTransferenciasH.php?idHerramienta=${data}" class="ver-transferencias btn btn-info btn-sm">
    ...
    // Abrir modal transferencia
        $('#herramientasTable').on('click', '.transfer-btn', function() {
            ...
            $('#idHerramienta').val(idHerramienta); // Este es el input oculto correcto
    ...
    // Procesar transferencia
        $('#formTransferencia').on('submit', function(e) {
            ...
            // Enviar los datos de la transferencia
                $.post('http://localhost/inventario-test/transferirHerramientas.php', $(this).serialize(), function(response) {
                    try {
        ...
        if (response.status === "success") {
            console.log("Datos de la herramienta:", response.data);
        } else {
                ...
                if (confirm('¿Está seguro de que desea eliminar esta herramienta?')) {
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            $('#herramientasTable').DataTable().ajax.reload();
                        } else {
