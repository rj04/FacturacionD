<?php
// c:\UniServerZ\www\inventario-test\listarResidentes_dt.php
...
$orderColumnIndex = isset($_GET['order'][0]['column']) ? intval($_GET['order'][0]['column']) : 1; // Ordenar por residente por defecto
...
$columnMap = [
    0 => 'residente', // JS Col 0 (Residente)
    1 => 'telefono',  // JS Col 1 (Teléfono)
];
$orderColumnName = $columnMap[$orderColumnIndex] ?? $columnMap[1]; // Columna por defecto

// --- Construir Consulta SQL Base ---
$baseSql = " FROM residentes ";
...
if (!empty($customSearchValue)) {
    $whereSql = " WHERE (LOWER(residente) LIKE LOWER(?) OR LOWER(telefono) LIKE LOWER(?)) ";
...
$totalRecordsSql = "SELECT COUNT(idResidente) as total " . $baseSql;
...
$filteredRecordsSql = "SELECT COUNT(idResidente) as total " . $baseSql . $whereSql;
...
if ($stmtFiltered) {
    ...
    } else { error_log("Error ejecutando conteo filtrado en listarResidentes_dt.php: " . $stmtFiltered->error); }
    $stmtFiltered->close();
} else { error_log("Error preparando conteo filtrado en listarResidentes_dt.php: " . $mysqli->error); }

// --- Construir Consulta Principal para Obtener Datos ---
$dataSql = "SELECT idResidente, residente, telefono " . $baseSql . $whereSql
...
if ($stmtData) {
    ...
    if ($stmtData->execute()) {
        ...
        else { error_log("Error obteniendo resultados de datos en listarResidentes_dt.php: " . $mysqli->error); }
    } else { error_log("Error ejecutando consulta de datos en listarResidentes_dt.php: " . $stmtData->error); }
    $stmtData->close();
} else { error_log("Error preparando consulta de datos en listarResidentes_dt.php: " . $mysqli->error); }
